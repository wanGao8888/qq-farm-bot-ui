...
function getStealPlantBlacklistSet() {
  ...
  return new Set(
    normalizeStealPlantBlacklist(automation.friend_steal_blacklist),
  )
}
...
function addFriendToBlacklist(friendGid, friendName, reason = '') {
  ...
  logWarn(
    '好友',
    `检测到封禁好友，已自动加入黑名单: ${friendName || `GID:${gid}`}`,
    {
      module: 'friend',
      event: '加黑名单',
      result: 'auto_blocked',
...
function getIdleFriendProbeBudget(priorityCount) {
  ...
  if (count >= Math.floor(IDLE_FRIEND_PROBE_SKIP_THRESHOLD / 2))
    return IDLE_FRIEND_PROBE_REDUCED_BATCH_SIZE
  return IDLE_FRIEND_PROBE_BATCH_SIZE
}

function selectIdleFriendProbeCandidates(
  candidates,
  budget,
  nowMs = Date.now(),
) {
...
function markIdleFriendProbeCooldown(friendGid, hadAction, nowMs = Date.now()) {
  ...
  idleFriendProbeCooldownUntil.set(
    ...
    nowMs +
      (hadAction
        ? IDLE_FRIEND_PROBE_HIT_COOLDOWN_MS
        : IDLE_FRIEND_PROBE_MISS_COOLDOWN_MS),
  )
}
...
async function getApplications() {
  const body = types.GetApplicationsRequest.encode(
...
async function enterFriendFarm(friendGid) {
  const body = types.VisitEnterRequest.encode(
    types.VisitEnterRequest.create({
...
async function leaveFriendFarm(friendGid) {
  const body = types.VisitLeaveRequest.encode(
    types.VisitLeaveRequest.create({
      host_gid: toLong(friendGid),
    }),
  ).finish()
  try {
...
async function putInsects(friendGid, landIds) {
  return putPlantItems(
    friendGid,
    landIds,
    types.PutInsectsRequest,
    types.PutInsectsReply,
    'PutInsects',
  )
}
...
function analyzeFriendLands(lands, myGid, friendName = '', options = {}) {
  const stealPlantBlacklistSet =
    options.stealPlantBlacklistSet instanceof Set
      ? options.stealPlantBlacklistSet
      : null
  ...
  for (const land of lands) {
    ...
    if (phaseVal === PlantPhase.MATURE) {
      if (plant.stealable) {
        ...
        if (
          stealPlantBlacklistSet &&
          plantId > 0 &&
          stealPlantBlacklistSet.has(plantId)
        ) {
          continue
        }
...
async function doFriendOperation(friendGid, opType) {
  ...
  try {
    ...
    if (opType === 'steal') {
      ...
      const maxNum =
        precheck.canStealNum > 0
          ? precheck.canStealNum
          : status.stealable.length
    ...
    if (opType === 'bad') {
      ...
      if (status.canPutBug.length) {
        ...
        bugCount = bugRet.ok
        failDetails = failDetails.concat(
          (bugRet.failed || []).map((f) => `放虫#${f.landId}:${f.reason}`),
        )
      ...
      if (status.canPutWeed.length) {
        ...
        failDetails = failDetails.concat(
          (weedRet.failed || []).map((f) => `放草#${f.landId}:${f.reason}`),
        )
...
async function visitFriend(friend, totalActions, myGid, options = {}) {
  ...
  if (!helpEnabled) {
    ...
    for (const op of helpOps) {
      const allowByExp =
        !stopWhenExpLimit || (canGetExpByCandidates(op.expIds) && canGetHelpExp)
  ...
  if (actions.length > 0) {
    log('好友', `${name}: ${actions.join('/')}`, {
      ...
      friendGid: gid,
      actions,
      scanType: friend && friend.isProbe ? 'probe' : 'priority',
    })
  }
...
async function checkFriends() {
  ...
  try {
    ...
    for (const f of friends) {
      ...
      if (
        String(f.name || '').trim() === '小小农夫' ||
        String(f.remark || '').trim() === '小小农夫'
      )
        continue
      ...
      const hasHelpAction =
        helpScanEnabled && (dryNum > 0 || weedNum > 0 || insectNum > 0)
      ...
      if (hasStealAction || hasHelpAction) {
        ...
        (autoBadEnabled && canPutBugOrWeed) ||
        helpScanEnabled ||
        stealEnabled
      ) {
    ...
    const probeFriends = selectIdleFriendProbeCandidates(
      idleProbeCandidates,
      idleProbeBudget,
    )
    ...
    if (summary.length > 0) {
      log('好友', `巡查 ${friendsToVisit.length} 人 → ${summary.join('/')}`, {
        ...
        previewTargets: priorityFriends.length,
        probeTargets: probeFriends.length,
        probed: probeVisitedCount,
        summary,
      })
    }
...
async function friendCheckLoop() {
  ...
  friendScheduler.setTimeoutTask(
    'friend_check_loop',
    Math.max(0, CONFIG.friendCheckInterval),
    () => friendCheckLoop(),
  )
}
...
function startFriendCheckLoop(options = {}) {
  ...
  if (!externalSchedulerMode) {
    ...
    friendScheduler.setTimeoutTask('friend_check_loop', 5000, () =>
      friendCheckLoop(),
    )
  }
