...
async function waterLand(landIds) {
  const state = getUserState()
  return sendPlantRequest(
    types.WaterLandRequest,
    types.WaterLandReply,
    'WaterLand',
    landIds,
    state.gid,
  )
...
async function insecticide(landIds) {
  const state = getUserState()
  return sendPlantRequest(
    types.InsecticideRequest,
    types.InsecticideReply,
    'Insecticide',
    landIds,
    state.gid,
  )
...
function getSlaveLandIds(land) {
  const ids = Array.isArray(land && land.slave_land_ids)
    ? land.slave_land_ids
    : []
...
function getDisplayLandContext(land, landsMap) {
  ...
  if (masterLand && hasPlantData(masterLand)) {
    const occupiedLandIds = [
      toNum(masterLand.id),
      ...getSlaveLandIds(masterLand),
    ].filter(Boolean)
    return {
      ...
      occupiedLandIds:
        occupiedLandIds.length > 0
          ? occupiedLandIds
          : [toNum(masterLand.id)].filter(Boolean),
    }
  }
...
function formatFertilizerLandTypes(types) {
  return normalizeFertilizerLandTypes(types).map(
    (type) => FERTILIZER_LAND_TYPE_LABELS[type] || type,
  )
}
...
async function runFertilizerByConfig(plantedLands = [], options = {}) {
  ...
  try {
    ...
    logWarn(
      '施肥',
      `${reasonLabel}：获取土地信息失败，按已知地块继续: ${e.message}`,
      {
        module: 'farm',
        event: eventName,
        result: 'error',
  ...
  const isAllLandTypesSelected =
    selectedLandTypes.length === ALL_FERTILIZER_LAND_TYPES.length
  ...
  if (landTypeById.size > 0) {
    normalTargets = filterLandIdsByTypes(
      planted,
      landTypeById,
      selectedLandTypes,
    )
  ...
  if (
    (fertilizerConfig === 'normal' || fertilizerConfig === 'both') &&
    ...
    if (fertilizedNormal > 0) {
      log(
        '施肥',
        `${reasonLabel}：已为 ${fertilizedNormal}/${normalTargets.length} 块地施普通化肥（范围: ${selectedLandTypeNames.join('、')}）`,
        {
          module: 'farm',
          event: eventName,
          result: 'ok',
  ...
  if (fertilizerConfig === 'organic' || fertilizerConfig === 'both') {
    ...
    if (landTypeById.size > 0) {
      organicTargets = filterLandIdsByTypes(
        organicTargets,
        landTypeById,
        selectedLandTypes,
      )
    ...
    if (fertilizedOrganic > 0) {
      log(
        '施肥',
        `${reasonLabel}：有机化肥循环施肥完成，共施 ${fertilizedOrganic} 次（范围: ${selectedLandTypeNames.join('、')}）`,
        {
          module: 'farm',
          event: eventName,
          result: 'ok',
...
async function upgradeLand(landId) {
  const body = types.UpgradeLandRequest.encode(
    types.UpgradeLandRequest.create({
...
async function unlockLand(landId, doShared = false) {
  const body = types.UnlockLandRequest.encode(
    types.UnlockLandRequest.create({
      land_id: toLong(landId),
      do_shared: !!doShared,
    }),
  ).finish()
...
async function plantSeeds(seedId, landIds, options = {}) {
  ...
  const maxPlantCount =
    Math.max(0, toNum(options.maxPlantCount) || 0) || Number.POSITIVE_INFINITY
  const pendingLandIds = new Set(
  ...
  for (const rawLandId of landIds) {
    ...
    try {
      ...
      const displayContext = getDisplayLandContext(
        selfLand || { id: landId },
        changedMap,
      )
      const occupiedIds =
        displayContext.occupiedLandIds.length > 0
...
async function getAvailableSeeds() {
  ...
  return list.sort((a, b) => {
    const av =
      a.requiredLevel === null || a.requiredLevel === undefined
        ? 9999
        : a.requiredLevel
    const bv =
      b.requiredLevel === null || b.requiredLevel === undefined
        ? 9999
        : b.requiredLevel
    return av - bv
  })
...
async function getLandsDetail() {
  try {
    ...
    for (const land of landsReply.lands) {
      ...
      const needWater =
        toNum(plant.dry_num) > 0 ||
        (toTimeSec(currentPhase.dry_time) > 0 &&
          toTimeSec(currentPhase.dry_time) <= nowSec)
      const needWeed =
        (plant.weed_owners && plant.weed_owners.length > 0) ||
        (toTimeSec(currentPhase.weeds_time) > 0 &&
          toTimeSec(currentPhase.weeds_time) <= nowSec)
      const needBug =
        (plant.insect_owners && plant.insect_owners.length > 0) ||
        (toTimeSec(currentPhase.insect_time) > 0 &&
          toTimeSec(currentPhase.insect_time) <= nowSec)
...
async function autoPlantEmptyLands(deadLandIds, emptyLandIds) {
  ...
  if (deadLandIds.length > 0) {
    try {
      ...
      log(
        '铲除',
        `已铲除 ${deadLandIds.length} 块 (${deadLandIds.join(',')})`,
        {
          module: 'farm',
          event: 'remove_plant',
          result: 'ok',
...
async function plantFromBagSeeds(landsToPlant, options = {}) {
  ...
  try {
    ...
    log(
      '背包',
      `获取到 ${bagSeeds.length} 种种子: ${bagSeeds.map((s) => `${s.name}x${s.count}`).join(', ') || '无'}`,
      {
        module: 'farm',
        event: 'bag_seeds_fetch',
        count: bagSeeds.length,
      },
    )
  } catch (e) {
  ...
  log(
    '背包',
    `用户优先级设置: ${priority.length > 0 ? priority.join(',') : '无(按等级排序)'}`,
    {
      module: 'farm',
      event: 'bag_priority',
      priority,
  ...
  if (!availableSeed) {
    // 检查是否有 1x1 种子
    const has1x1Seeds = sortedSeeds.some(
      (s) => s.count > 0 && (s.plantSize || 1) === 1,
    )
    if (!has1x1Seeds) {
  ...
  const seedLabel = isEvent
    ? `[活动] ${availableSeed.name}`
    : availableSeed.name
...
function sortBagSeedsByPriority(bagSeeds, priority) {
  ...
  return [...bagSeeds].sort((a, b) => {
    const pa = priorityMap.has(a.seedId)
      ? priorityMap.get(a.seedId)
      : Number.MAX_SAFE_INTEGER
    const pb = priorityMap.has(b.seedId)
      ? priorityMap.get(b.seedId)
      : Number.MAX_SAFE_INTEGER
...
async function plantFromShop(landsToPlant, state, options = {}) {
  ...
  log(
    '商店',
    `最佳种子: ${seedName} (${bestSeed.seedId}) 价格=${bestSeed.price}金币${growTimeStr}`,
    {
      module: 'warehouse',
      event: 'seed_pick',
  ...
  if (landFootprint > 1) {
    ...
    if (needCount <= 0) {
      log(
        '种植',
        `${seedName} 需要至少 ${landFootprint} 块空地才能合并种植，当前仅 ${landsToPlant.length} 块可用，已跳过`,
        {
          module: 'farm',
          event: 'plant_seed',
          result: 'skip',
  ...
  if (totalCost > state.gold) {
    logWarn(
      '商店',
      `金币不足! 需要 ${totalCost} 金币, 当前 ${state.gold} 金币`,
      {
        module: 'farm',
        event: 'seed_buy_skip',
    ...
    log(
      '商店',
      plantSize > 1
        ? `金币有限，只尝试种植 ${canBuy} 组 ${plantSize}x${plantSize} 作物`
        : `金币有限，只种 ${canBuy} 块地`,
    )
  }
  ...
  try {
    ...
    log(
      '购买',
      `已购买 ${boughtName}种子 x${needCount}, 花费 ${bestSeed.price * needCount} 金币`,
      {
        module: 'warehouse',
        event: 'seed_buy',
        result: 'ok',
  ...
  let plantedLands = []
  try {
    const { planted, plantedLandIds, occupiedLandIds } = await plantSeeds(
      actualSeedId,
      landsToPlant,
      { maxPlantCount: needCount },
    )
    const occupiedCount =
      occupiedLandIds.length > 0 ? occupiedLandIds.length : planted
    log(
      '种植',
      plantSize > 1
...
async function resolveRemovableHarvestedLands(harvestedLandIds, harvestReply) {
  const ids = Array.isArray(harvestedLandIds)
    ? harvestedLandIds.filter(Boolean)
    : []
  if (ids.length === 0) {
...
async function runFarmOperation(opType, options = {}) {
  ...
  if (
    opType === 'all' &&
    postHarvest &&
    Array.isArray(postHarvest.growing) &&
    postHarvest.growing.length > 0 &&
    isAutomationOn('fertilizer_multi_season')
  ) {
    const multiSeasonTargets = [
      ...new Set(postHarvest.growing.map((v) => toNum(v)).filter(Boolean)),
    ]
    if (multiSeasonTargets.length > 0) {
      try {
        await runFertilizerByConfig(multiSeasonTargets, {
          reason: 'multi_season',
        })
      } catch (e) {
